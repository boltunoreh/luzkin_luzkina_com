...
//ДОБАВЛЕНИЕ КОЛОНОК С ТАКСОНОМИЯМИ ДЛЯ КАСТОМНЫХ ТИПОВ ЗАПИСЕЙ
add_filter('manage_movies_posts_columns', 'set_edit_movies_columns'); //добавление колонки с таксономией для киношек
function set_edit_movies_columns($columns)
{
	$columns['cinema']='Cinema rating';
	return $columns;
}
add_action( 'manage_movies_posts_custom_column', 'custom_columns_movies'); 
function custom_columns_movies($column)
{
	global $post;
	switch($column)
	{
		case 'cinema':
		$terms = get_the_terms($post->ID, 'cinema');
...
add_filter('manage_places_posts_columns', 'set_edit_places_columns'); //добавление колонки с таксономией для togo
function set_edit_places_columns($columns)
{
	$columns['togo']='Part of the world';
	$columns['togo_type']='Type';
	return $columns;
}
add_action( 'manage_places_posts_custom_column', 'custom_columns_places'); 
function custom_columns_places($column)
{
	global $post;
	switch($column)
	{
		case 'togo':
		$terms = get_the_terms($post->ID, 'togo');
    $i = 0;
    foreach ( $terms as $term ) {
      if ($i > 0) { echo ", ";}
      echo $term->name;
      $i++;
		...
		break;
    case 'togo_type':
		$terms = get_the_terms($post->ID, 'togo_type');
    foreach ( $terms as $term ) {
      echo $term->name;
    }
		break;
...
add_filter('manage_videos_posts_columns', 'set_edit_videos_columns'); //добавление колонки с таксономией для video
function set_edit_videos_columns($columns)
{
	$columns['l-and-l-video']='Video tags';
	return $columns;
}
add_action( 'manage_videos_posts_custom_column', 'custom_columns_videos'); 
function custom_columns_videos($column)
{
	global $post;
	switch($column)
	{
		case 'l-and-l-video':
		$terms = get_the_terms($post->ID, 'l-and-l-video');
    $i = 0;
    foreach ( $terms as $term ) {
      if ($i > 0) { echo ", ";}
      echo $term->name;
...
add_filter('manage_caps_posts_columns', 'set_edit_caps_columns'); //добавление колонки с таксономией для caps
function set_edit_caps_columns($columns)
{
	$columns['beer-caps']='Categories';
	return $columns;
}
add_action( 'manage_caps_posts_custom_column', 'custom_columns_caps'); 
function custom_columns_caps($column)
{
	global $post;
	switch($column)
	{
    case 'beer-caps':
		$terms = get_the_terms($post->ID, 'beer-caps');
    foreach ( $terms as $term ) {
      echo $term->name;
    }
		break;
